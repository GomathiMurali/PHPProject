  ...
  <div id="wrapper">
    ...
    <div id="content-wrapper">
      ...
      <div class="container-fluid">
		...
		<?php
		    $sdsd=$_GET['invoice'];
			$resultas = $db->prepare("SELECT sum(total_amount) FROM salesform WHERE invoice= :a");
       ...
       ?></h3>
		...
		<?php 
	   include('connect.php');
	    $results = $db->prepare("SELECT max(id) FROM salesform");
        ...
        ?>
		<?php $y=date('y'); ?>
         <?php $vno="CS".$y.'-'. str_pad($dsdsdf+1,4,'0',STR_PAD_LEFT)?>
		 

          <label>Invoice: </label><input type="text" name="invoice" class = "form-control" style="width: 300px;" value="<?php echo $_GET['invoice']; ?>" />
		  ...
		  <input type="hidden" name="groupname" class = "form-control" value="<?php echo $_GET['groupname']; ?>" />
		   <input type="hidden" name="refinvoice" class = "form-control" value="<?php echo $_GET['refinvoice']; ?>" />
		...
		<label>From: </label><input type="text" name="fromname"  style="width: 300px;" class = "form-control" value="PPF" />
		<label>To: </label>
		
<br>
											<select name="supplier" style="width: 300px;" class="chzn-select">
 ...
 <?php
    $id=$_GET['invoice'];
    include('connect.php');
    $resultaz = $db->prepare("SELECT * FROM salesform WHERE invoice= :xzxz");
...
//echo 'Status : '.$rowaz['status'].'<br>';
    ...
    ?>
      ...
      <tbody>
       ...
       $id=$_GET['invoice'];
       include('connect.php');
       $result = $db->prepare("SELECT * FROM salesform WHERE invoice= :userid");
       ...
       for($i=0; $row = $result->fetch(); $i++){
		...
		<td><a href="deletesaleslist.php?id=<?php echo $row['id']; ?>&invoice=<?php echo $_GET['invoice']; ?>&qty=<?php echo $row['qty'];?>&code=<?php echo $row['product'];?>&bank=<?php echo $bank;?>&refinvoice=<?php echo $refinvoice;?>&groupname=<?php echo $groupname;?>"> Delete</a></td>
...
<a  class="btn btn-primary"  href="printpur1.php?invoice=<?php echo $_GET['invoice'];?>&total=<?php echo $fgfg; ?>&name=<?php echo $supp; ?>&date=<?php echo $duedate; ?>&phone=<?php echo $phone;?>&bank=<?php echo $bank;?>&refinvoice=<?php echo $refinvoice;?>">Print</a>
