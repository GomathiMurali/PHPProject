...
<div class="content" id="content">
  ...
  <center><h5> SALES RETURN BILL </h5></center>
  <div>
    <?php
    $id=$_GET['id'];
    include('connect.php');
    $resultaz = $db->prepare("SELECT * FROM sales WHERE invoice_number= :xzxz");
  ...
  <tr>
  <td>
  Customer</td><td>:</td><td><?php echo $supp ?></td></tr>
  <tr>
  <td>Date</td><td>:</td><td><?php echo date("d/m/y") ?></td></tr>
  </table>
  <table align="right">
  <tr>
  <td>Invoice No</td><td>:</td><td><?php echo $invoicenumber ?></td></tr>
  <tr>
  <td>Sales Date</td><td>:</td><td><?php echo $dateorder ?></td></tr>
  </table>
  </div>
  </div>
  </div>
  ...
  <tbody>
   ...
   $pname=$_GET['pname'];
   include('connect.php');
   $result = $db->prepare("SELECT * FROM sales_order WHERE invoice= :userid AND product='$pname'");
   ...
   for($i=0; $row = $result->fetch(); $i++){
    ...
    $sdsd=$_GET['id'];
	$pname=$_GET['pname'];
    $resultas = $db->prepare("SELECT sum(total_amount) FROM sales_order WHERE invoice= :a AND product='$pname'");
...
<a href = "saleslist.php" class="btn btn-primary btn-m " >
  Back    
</a></center>
