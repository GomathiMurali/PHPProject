<?php
	include('connect.php');
	$id=$_GET['id'];
	$result = $db->prepare("SELECT * FROM mess_supervisor WHERE mess_id= :userid");
...
<body>
<form action="saveeditmesspurchase.php" method="post" class = "form-group" enctype="multipart/form-data">
<div id="ac">
...
<!--<input type="text" class = "form-control"  name="name" value="<?php echo $row['catagory']; ?>" />-->
...
<span>Amount : </span><input type="text" class = "form-control"  name="amount" value="<?php 
				$amt=$row['amount'];
				if($amt!=0){echo $amt;}else {echo $row['debit_amount'];} 
				?>" />
				<br>
<span>Status : </span>
...
<?php include('connect.php');
	$id=$_GET['id'];
	$result = $db->prepare("SELECT * FROM mess_file WHERE mess_id= :userid");
	$result->bindParam(':userid', $id);
	$result->execute();
