<?php
	...
	/* $from1	=$_GET['from1'];
	$to1	=$_GET['to1']; */
	$result = $db->prepare("SELECT * FROM expensepending WHERE pending_id= :userid");
...
<body>
<form action="saveeditexpensepending.php" method="post" class = "form-group" enctype="multipart/form-data">
<div id="ac">
<input type="hidden" class = "form-control" name="memi" value="<?php echo $id; ?>" />
<!--<input type="hidden" class = "form-control" name="from1" value="<?php echo $from1; ?>" />
<input type="hidden" class = "form-control" name="to1" value="<?php echo $to1; ?>" />-->
...
<span>Amount : </span><input type="text" class = "form-control"  name="amount" value="<?php 
				$amt=$row['amount'];
				if($amt!=0){echo $amt;}else {echo $row['paidamount'];} 
				?>" />
				<br>
<span>Status : </span>
<input type="text" class = "form-control"  name="status" value="<?php echo $row['status']; ?>" readonly>
<span>Transaction Type :</span><input type="text" class = "form-control"  name="trans" value="<?php echo $row['trans']; ?>" readonly>
...
<?php include('connect.php');
	$id=$_GET['id'];
	$result = $db->prepare("SELECT * FROM bill_file WHERE pending_id= :userid");
	$result->bindParam(':userid', $id);
	$result->execute();
	for($i=0; $row1 = $result->fetch(); $i++){
	?>
	<!--<img src="<?php echo $row1['bill_attach']; ?>" height="100" width="100"/>  -->
	<a href="<?php echo $row1['bill_attach']; ?>" target='_blank' style="background-color:#337ab7;color:#FFFFFF;">Bill Attachement</a>
	<?php } ?></span>
	
<?php
$res = $db->prepare("SELECT * FROM expensepending_balance WHERE pending_id= :userid");
...
<input type="hidden" class = "form-control" name="bal" value="<?php echo $bal; ?>" />
<input type="hidden" class = "form-control" name="oldamount" value="<?php 
				$amt=$row['amount'];
				if($amt!=0){echo $amt;}else {echo $row['paidamount'];} 
				?>" />
				<br>
