...
<body>
    ...
    <div id="page-wrapper">
        <div class="container-fluid">
            <div class="row">
                <div class="col-lg-12">
                    <h2 class="page-header">Mess Purchase Report</h2>
                </div>
                ...
                <table width="100%" class="table table-striped table-bordered table-hover">
                    ...
                    <tbody>
						...
						$c=$from1;
					    $d=$to1;
                        $result = $db->prepare("SELECT description,(sum(amount)+sum(debit_amount)) AS amount FROM mess_supervisor where STR_TO_DATE(date, '%d-%m-%Y') BETWEEN STR_TO_DATE('$c', '%d-%m-%Y') AND STR_TO_DATE('$d', '%d-%m-%Y') and status!='advance_amount' group by description ORDER BY id asc");
					...
					<thead>
                            <tr>
                                ...
                                <th colspan="1" style="border-top:1px solid #999999"> 
                                    ...
                                    $results = $db->prepare("SELECT sum(amount) as balance FROM mess_supervisor where STR_TO_DATE(date, '%d-%m-%Y') BETWEEN STR_TO_DATE('$c', '%d-%m-%Y') AND STR_TO_DATE('$d', '%d-%m-%Y') and status!='advance_amount'");
                                    $results->execute();
                                    for($i=0; $rows = $results->fetch(); $i++){
                                        $dsdsd=$rows['balance'];
                                    ...
                                    <?php
                                    $results = $db->prepare("SELECT sum(debit_amount) as balance FROM mess_supervisor where STR_TO_DATE(date, '%d-%m-%Y') BETWEEN STR_TO_DATE('$c', '%d-%m-%Y') AND STR_TO_DATE('$d', '%d-%m-%Y') and status!='advance_amount'");
                                    $results->execute();
                                    for($i=0; $rows = $results->fetch(); $i++){
                                        $dsdsds=$rows['balance'];
