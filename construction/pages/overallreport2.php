        ...
        <div class="container-fluid">
            <div class="row">
                ...
                <div id="maintable" >
					...
					<tr bgcolor=" #85C1E9 ">
					<th colspan="4"><h4>Date:&nbsp;&nbsp;<?php echo $from1;?>&nbsp;&nbsp;to&nbsp;&nbsp;<?php echo $to1;?></h4></th></tr></thead>
					
					<?php
                            include('connect.php');
							$i=$day_before;
							$n='31-08-2018';
                            $result = $db->prepare("SELECT sum(amount) FROM income WHERE STR_TO_DATE(date, '%d-%m-%Y') BETWEEN STR_TO_DATE('$n', '%d-%m-%Y') AND STR_TO_DATE('$i', '%d-%m-%Y')");
					...
					<?php
							...
							$e=$day_before;
							$x='31-08-2018';
                            $result = $db->prepare("SELECT sum(amount) FROM expense WHERE STR_TO_DATE(date, '%d-%m-%Y') BETWEEN STR_TO_DATE('$x','%d-%m-%Y') AND STR_TO_DATE('$e', '%d-%m-%Y')");
					...
					<th colspan="4"><h4>Opening balance in bank:&nbsp;&nbsp;<?php echo formatMoney($obb,true);?></h4></th></tr></thead>
						...
						<thead>
                            <tr>
                                <th colspan="2" style="border-top:1px solid #999999"> Bank Opening Balance</th>
                            </tr>
                        </thead>
					     <thead>
                            <tr>
                        ...
                        <thead>
						 ...
						 <?php
							...
							$c=$from1;
							$d=$to1;
							$res = $db->prepare("SELECT catagory_name,sum(amount) FROM income WHERE STR_TO_DATE(date, '%d-%m-%Y') BETWEEN STR_TO_DATE('$c', '%d-%m-%Y') AND STR_TO_DATE('$d', '%d-%m-%Y') GROUP BY catagory_name");
							$res->bindParam(':c', $c);
                            $res->bindParam(':d', $d);
							$res->execute();
						   ...
						   <?php
						    $s=$row['catagory_name'];
                            $result = $db->prepare("SELECT subcatagory,sum(amount) FROM income WHERE catagory_name=:s AND STR_TO_DATE(date, '%d-%m-%Y') BETWEEN STR_TO_DATE('$c', '%d-%m-%Y') AND STR_TO_DATE('$d', '%d-%m-%Y') GROUP BY subcatagory");
                        ...
                        <thead>
                            <tr>
                                ...
                                <td  align="right" colspan="1" style="font-weight:bold"> 
                                    ...
                                    $results = $db->prepare("SELECT sum(amount) FROM income WHERE STR_TO_DATE(date, '%d-%m-%Y') BETWEEN STR_TO_DATE('$c', '%d-%m-%Y') AND STR_TO_DATE('$d', '%d-%m-%Y') ");
					...
					<table width="100%"   id="dataTables-example" border="1">
                        <thead>
						 ...
						 <?php
							...
							$c=$from1;
							$d=$to1;
							$res = $db->prepare("SELECT catagory_name,sum(amount) FROM expense WHERE STR_TO_DATE(date, '%d-%m-%Y') BETWEEN STR_TO_DATE('$c', '%d-%m-%Y') AND STR_TO_DATE('$d', '%d-%m-%Y') GROUP BY catagory_name");
							$res->bindParam(':c', $c);
                            $res->bindParam(':d', $d);
							$res->execute();
							for($i=0; $row = $res->fetch(); $i++){
						  ...
						  <?php
						    $s=$row['catagory_name'];
                            $result = $db->prepare("SELECT subcatagory,sum(amount) FROM expense WHERE catagory_name=:s AND STR_TO_DATE(date, '%d-%m-%Y') BETWEEN STR_TO_DATE('$c', '%d-%m-%Y') AND STR_TO_DATE('$d', '%d-%m-%Y') GROUP BY subcatagory");
							$result->bindParam(':s', $s);
                            $result->execute();
                            for($i=0; $row = $result->fetch(); $i++){
                                ?>
                        ...
                        <thead>
                            <tr>
                                ...
                                <td colspan="1"  align="right" style="font-weight:bold"> 
                                    ...
                                    $results = $db->prepare("SELECT sum(amount) FROM expense WHERE STR_TO_DATE(date, '%d-%m-%Y') BETWEEN STR_TO_DATE('$c', '%d-%m-%Y') AND STR_TO_DATE('$d', '%d-%m-%Y') ");
									$results->bindParam(':c', $c);
                                    $results->bindParam(':d', $d);
                                    $results->execute();
                                    for($i=0; $rows = $results->fetch(); $i++){
						...
						<thead>
                            <tr>
                                <th colspan="5" style="border-top:1px solid #999999">Bank Closing Balance</th>
                            </tr>
                        </thead>
					     <thead>
                            <tr>
					...
					<?php
						...
						<th colspan="4"><h4>Closing balance in hand:&nbsp;&nbsp;<?php echo formatMoney($cbh,true);?></h4></th></tr>
						<tr bgcolor=" #AF7AC5">
						<th colspan="4"><h4>Closing balance in Bank:&nbsp;&nbsp;<?php echo formatMoney($cbb,true);?></h4></th></tr>
						<tr>
						<th colspan="4"><h4>Closing balance:&nbsp;&nbsp;<?php echo formatMoney($cb,true);?></h4></th></tr>
						</table>
