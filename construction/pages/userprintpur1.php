...
<div class="content" id="content">
 ...
 <hr class="new1">
  ...
  <tbody>
     ...
     $id=$_GET['invoice'];
     include('connect.php');
	 $resultpp = $db->prepare("SELECT * FROM salesform WHERE invoice= :userid");
     ...
     for($i=0; $row = $resultpp->fetch(); $i++){
      ...
      $sdsd=$_GET['invoice'];
      $resultas = $db->prepare("SELECT sum(total_amount),sum(amount),sum(cgstamt),sum(sgstamt),sum(igstamt) FROM salesform WHERE invoice= :a");
